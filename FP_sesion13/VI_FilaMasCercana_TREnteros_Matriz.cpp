...
public:
    ...
    {
        SecuenciaEnteros fila_secuencia;
        
        if (indice_fila >= 0 && indice_fila < filas_utilizadas) 
        {
            for (int j = 0; j < cols_utilizadas; ++j) 
            {
                fila_secuencia.Aniade(matriz_privada[indice_fila][j]);
            }
        }
        return fila_secuencia;
    }
    ...
    void Aniade(SecuenciaEnteros fila_nueva) 
    {
        ...
        {
            for (int j = 0; j < cols_utilizadas; ++j) {
                matriz_privada[filas_utilizadas][j] = fila_nueva.Elemento(j);
            }
            filas_utilizadas++;
        }
    }
    ...
    void Inserta(int num_fila, SecuenciaEnteros fila_nueva) 
    {
        if (filas_utilizadas < NUM_FILS && 
            ...
            fila_nueva.TotalUtilizados() == cols_utilizadas) 
        {
            // Desplazar filas hacia abajo para hacer hueco
            ...
            for (int j = 0; j < cols_utilizadas; ++j) 
            {
                matriz_privada[num_fila][j] = fila_nueva.Elemento(j);
            }
            filas_utilizadas++;
        }
    }
