...
/*
    ...
    las mezcla en un único array ordenado y sin repeticiones. Al final
    ...
    introducidas estén ordenadas antes de mezclar los arrays.

    Entradas:
        - Dos secuencias de enteros ordenados ascendentemente.
    Salidas:
        - La secuencia mezclada, ordenada y sin repeticiones.
*/
/***************************************************************************/
...
/*
    ...
    Si hay elementos repetidos, solo se copia uno en el array de destino.
    Parámetros:
        ...
        utiles_mezcla: Ref. al número de elementos en mezcla. Se modifica.
*/
/***************************************************************************/

void Mezclar(int arr1[], int utiles1, int arr2[], int utiles2, 
             int mezcla[], int &utiles_mezcla)
{   
    ...
    {
        int elemento_a_copiar;

        // Compara los elementos actuales de cada array y elige el menor.
        if (arr1[actual_1] < arr2[actual_2])
        {
            elemento_a_copiar = arr1[actual_1];
            actual_1++;
        } else if (arr2[actual_2] < arr1[actual_1])
        {
            elemento_a_copiar = arr2[actual_2];
            actual_2++;
        } else { // Son iguales
            elemento_a_copiar = arr1[actual_1];
            actual_1++;
        ...
        // Añadir el elemento si es el primer elemeno o si no es un duplicado 
        // del último añadido.
        if (utiles_mezcla == 0 || mezcla[utiles_mezcla - 1] 
                                  != elemento_a_copiar) 
        {
            mezcla[utiles_mezcla] = elemento_a_copiar;
            utiles_mezcla++;
        }
    }
    ...
    {
        // Añadir el elemento si es el primer elemento o si no es un duplicado 
        // del último añadido.
        if (utiles_mezcla == 0 || mezcla[utiles_mezcla - 1] != arr1[actual_1]) 
        {
            mezcla[utiles_mezcla] = arr1[actual_1];
            utiles_mezcla++;
        }
        actual_1++;
    }
    ...
    while (actual_2 < utiles2) 
    {
        // Añadir el elemento si es el primer elemento o si no es un duplicado 
        // del último añadido.
        if (utiles_mezcla == 0 || mezcla[utiles_mezcla - 1] != arr2[actual_2]) 
        {
            mezcla[utiles_mezcla] = arr2[actual_2];
            utiles_mezcla++;
        }
        actual_2++;
    }
...
int main(){
    ...
    Mezclar(v1, utiles1, v2, utiles2, mezcla, utiles_mezcla);
